...
with st.expander("🎨 Reformula tu texto con otro tono"):
    ...
    tone = st.selectbox("Selecciona un tono nuevo:", ["Más positivo 😄", "Más profesional 💼", "Más informal 😎"])

    if text3 and st.button("Reformular texto"):
        blob3 = TextBlob(text3)
        trans_es = translator.translate(text3, src="auto", dest="es").text
        
        if tone == "Más positivo 😄":
            st.success(f"✨ {trans_es.capitalize()}... pero imagina que lo dices con una sonrisa y optimismo.")
        elif tone == "Más profesional 💼":
            st.success(f"💼 {trans_es.capitalize()}... podrías decirlo con más cortesía y precisión.")
        elif tone == "Más informal 😎":
            st.success(f"😎 {trans_es.capitalize()}... súmale expresiones relajadas, como si hablaras con amigos.")
